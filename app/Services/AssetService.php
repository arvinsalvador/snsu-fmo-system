...
class AssetService
{
    ...
    {
        return DB::transaction(fn (): Asset => Asset::query()->create($data)->load($this->relations()));
    }
    ...
    {
        $sort = in_array($filters['sort'] ?? '', ['asset_code', 'brand', 'model', 'serial_number', 'purchase_date', 'warranty_until', 'status', 'created_at'], true) ? $filters['sort'] : 'asset_code';
        $direction = ($filters['direction'] ?? 'asc') === 'desc' ? 'desc' : 'asc';

        return Asset::query()->with(['category', 'building', 'floor', 'room'])
