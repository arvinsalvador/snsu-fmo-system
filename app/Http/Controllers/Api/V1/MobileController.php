...
class MobileController extends Controller
{
    ...
    {
        return response()->json(['success' => true, 'message' => 'Reference data retrieved successfully.', 'data' => ['buildings' => Building::where('is_active', true)->get(['uuid', 'name']), 'floors' => Floor::where('is_active', true)->get(['uuid', 'building_id', 'floor_name']), 'rooms' => Room::where('is_active', true)->get(['uuid', 'floor_id', 'room_code', 'name']), 'priorities' => Priority::where('is_active', true)->get(['uuid', 'name', 'level']), 'work_order_categories' => WorkOrderCategory::where('is_active', true)->get(['uuid', 'name']), 'asset_categories' => AssetCategory::where('is_active', true)->get(['uuid', 'name']), 'maintenance_types' => MaintenanceType::where('is_active', true)->get(['uuid', 'name']), 'inventory_categories' => InventoryCategory::where('is_active', true)->get(['uuid', 'name'])], 'meta' => ['cached_for_seconds' => 300]]);
    }
