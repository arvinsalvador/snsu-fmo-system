<x-app-layout>
    <x-slot name="header"><h1 class="text-2xl font-semibold tracking-tight">Preventive Maintenance</h1></x-slot>
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div class="flex gap-2">
            @can('export', App\Models\MaintenanceSchedule::class)<a href="{{ route('maintenance-schedules.export', request()->query()) }}" class="rounded-md border border-zinc-300 bg-white px-4 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-100">Export CSV</a>@endcan
            @can('create', App\Models\MaintenanceSchedule::class)<a href="{{ route('maintenance-schedules.create') }}" class="rounded-md bg-zinc-950 px-4 py-2 text-sm font-semibold text-white hover:bg-zinc-800">New Schedule</a>@endcan
        </div>
    </div>
                        ...
                        <tr class="align-top">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex justify-end gap-2">
                                    @can('complete', $schedule)<a href="{{ route('maintenance-schedules.show', $schedule) }}" class="rounded-md border border-emerald-300 px-2 py-1 text-xs font-semibold text-emerald-700 hover:bg-emerald-50">Complete</a>@endcan
                                    @can('update', $schedule)<a href="{{ route('maintenance-schedules.edit', $schedule) }}" class="rounded-md border border-zinc-300 px-2 py-1 text-xs font-semibold text-zinc-700 hover:bg-zinc-100">Edit</a>@endcan
                                </div>
                            </td>
                        </tr>
                    @empty
